...
try {
    ...
    if ($selectedCertIndex -match '^\d+$' -and [int]$selectedCertIndex -lt $store.Certificates.Count) {
        ...
        if ($selectedCert.Extensions) {
            ...
            $policyExt = $selectedCert.Extensions | Where-Object { $_.Oid.FriendlyName -eq 'Certificate Policies' }
            if ($policyExt) {
                Write-Host "Certificate Policies:" -ForegroundColor White
                $policies = $policyExt.Format($true)
                # Split the policies string into individual policies and clean up the format
                $policies -split "`n" | Where-Object { $_ -match 'Policy Identifier=([\d\.]+)' } | ForEach-Object {
                    $policyOid = $matches[1]
                    Write-Host "    Policy OID: " -NoNewline
                    Write-Host $policyOid -ForegroundColor White
                }
            } else {
                Write-Host "Certificate Policies: " -NoNewline
                Write-Host "Not Present" -ForegroundColor Red
            }
        }
